...
async def test_get_reviews_no_reviews(client: AsyncClient, create_test_book):
    ...
    async def test_get_reviews_unauthenticated(client: AsyncClient, create_test_book):
        ...
        async def create_test_user(test_db: AsyncSession):
            async def _create_test_user(user_id: str):
                user = models.User(id=user_id, username="testuser")
                test_db.add(user)
                await test_db.commit()
        ...
        @pytest.mark.asyncio
        async def test_add_review(client: AsyncClient, test_db: AsyncSession, create_test_book, create_test_user):
            # Arrange
            book = await create_test_book(book_id=1)
            user = await create_test_user(user_id="user1")
            review_data = {"rating": 5, "comment": "Great book!"}

            # Act
            response = await client.post(f"/1/reviews", json=review_data, headers={"Authorization": f"Bearer {user.id}"})
        ...
        async def test_add_review_book_not_found(client: AsyncClient, create_test_user):
            ...
            # Act
            response = await client.post(f"/999/reviews", json=review_data, headers={"Authorization": f"Bearer {user.id}"})
        ...
        @pytest.mark.asyncio
        async def test_get_reviews(client: AsyncClient, test_db: AsyncSession, create_test_book, create_test_user):
            # Arrange
            book = await create_test_book(book_id=1)
            user = await create_test_user(user_id="user1")
            review = models.Review(rating=5, comment="Great book!", book_id=book.id, user_id=user.id)
            test_db.add(review)
            await test_db.commit()
            await test_db.refresh(review)

            # Act
            response = await client.get(f"/1/reviews", headers={"Authorization": f"Bearer {user.id}"})
        ...
        @pytest.mark.asyncio
        async def test_get_reviews_no_reviews(client: AsyncClient, create_test_book, create_test_user):
            # Arrange
            book = await create_test_book(book_id=1)
            user = await create_test_user(user_id="user1")
